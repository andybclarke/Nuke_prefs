...
#
#  addition v03:
#  layer renamer - if layer name not in renameArr then leaf it as it is
#  create a shuffle node for first read in queue and shuffle in corresponding layer
#
#  instalation
#
...
# 'pattern recognized' : 'rename the whole layer to this one'
global renameArr
renameArr = {
             'AO' : 'AO',
...
def MultiChannelCombine():
    ...
    if len(images)>1:
        ...
        if noread==0:
                ...
                if isEmpty>0:
                    ...
                    if allUnder30Char==0:
                        for img in images:
                            ...
                            newLayer = nuke.Layer(channelNames[i],[r_temp, g_temp, b_temp, a_temp])   
                            
                            if i==0:
                                shuffle1 = nuke.nodes.Shuffle()
                                shuffle1.setName("shuffle_%s" % channelNames[i])
            ...
            nuke.message("Please make sure to select SHOTGUN READ nodes only")
    else:
        nuke.message("Please select at least 2 SgtkRead Nodes")
